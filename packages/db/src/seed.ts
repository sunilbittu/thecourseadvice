...
async function seed() {
  ...
  await db.delete(schema.scholarlyTools);
  await db.delete(schema.featuredAssets);
  await db.delete(schema.resourceCategories);
  await db.delete(schema.courseViews);
  await db.delete(schema.leads);
  await db.delete(schema.shortlists);
  ...
  await db.insert(schema.users).values(
    usersData.map((u) => ({
      ...
      role: u.role as "student" | "institution-admin" | "superadmin",
      avatar: u.avatar,
      bio: u.bio,
